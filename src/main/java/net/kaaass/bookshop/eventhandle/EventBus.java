...
public class EventBus implements IEventExceptionHandler {
    ...
    private void register(Class<?> eventType, Object target, Method method) {
        try {
            ...
            ArrayList<IEventListener> others;
            if (pluginListeners.containsKey(target)) {
                others = pluginListeners.get(target);
            } else {
                others = new ArrayList<>();
                pluginListeners.put(target, others);
            }
            others.add(listener);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void register(String pluginId, String eventClass, EventPriority priority, String codeStr) {
        try {
            ...
            ArrayList<IEventListener> others;
            if (pluginListeners.containsKey(pluginId)) {
                others = pluginListeners.get(pluginId);
            } else {
                others = new ArrayList<>();
                pluginListeners.put(pluginId, others);
            }
            others.add(listener);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void unregister(String pluginId) {
        val list = pluginListeners.remove(pluginId);
        if (list == null) {
            return;
        }
        for (val listener : list) {
    ...
    public boolean post(Event event) {
        log.info("触发事件：{}", event.getClass().getName());
    ...
    private static Set<Class<?>> getSuperAndMe(final Class<?> clazz) {
